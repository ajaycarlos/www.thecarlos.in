...
function initializeSystemFailure() {
    const triggerButton = document.getElementById('system-failure-button');
    ...
    if (!triggerButton) return; // Exit if the button isn't on the page
    ...
    const runFailureSequence = async () => {
        // 1. Glitch the text
        const textElements = document.querySelectorAll('#knowledge-bit-text, #knowledge-explanation, #live-clock, #footer-text');
        textElements.forEach(el => el.classList.add('text-glitch'));

        await delay(600); // Glitch duration

        // 2. Fade to black
        textElements.forEach(el => el.classList.remove('text-glitch'));
        overlay.style.visibility = 'visible';
        overlay.style.opacity = '1';

        await delay(500); // Wait for fade to black

        // 3. Flash "SYSTEM FAILURE"
        failureText.classList.add('flashing');
        
        await delay(3000); // Duration of failure message

        // 4. Show "System Rebooted"
